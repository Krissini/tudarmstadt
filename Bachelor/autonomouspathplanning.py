...
#
# This is done by taking the differential equations,
...
################################################################################
# Function to populate & invert Jacobian matrix.
# The best way to think of this is to use an analogy with simpler functions.
# If we define f(x) = 3x^2 - 2x - 7, this part of the script would be
# functionally equivalent to computing 1/f'(x). But in this script, the output
# of f(x) is a vector and the input x is also a vector.
...
def pathplanningbvpsolver(ALPHA,BETA,F,G,P0x,P0y,MAXITER,TOL,N):
    ...
    while ((error >= TOL) & (count < MAXITER)):
        ...
        # I print here to see the progress of the script as it goes.
        print(count,error)

    return solnew
################################################################################
# Initialize symbolic variables - using symbolic expressions will allow us to
# take the derivative of expressions within the code. (sympy)
...
xinterval = np.array([0,10]) # Range of x-coordinates for obstacles (where they can be placed)
yinterval = np.array([0,10]) # Range of y-coordinates for obstacles (where they can be placed)
M = 27  # Number of obstacles 1 1 1
...
# SET obstacles
obstaclesx = [-2, -3, -4, 9, 8, 7, -1, 0, 1, 1, 1, 1, 1, 0, -1, 10, 10,
              10, 10, 10, 10, 10, 10, 10, 10, -4, -4, -4, -4, -4] # x-coordinates of obstacles
obstaclesy = [-4, -4, -4, 14, 14, 14, 0, 0, 0, -1, -2, -3, -4, -4, -4, 14, 13,
              12, 11, 10, 9, 8, 7, 6, 6.5, -3, -2.5, -2, -1.5, -1] # y-coordinates of obstacles
