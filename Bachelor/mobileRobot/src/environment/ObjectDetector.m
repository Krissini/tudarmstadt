classdef ObjectDetector < matlab.System & matlab.system.mixin.CustomIcon & matlab.system.mixin.Propagates
    ...
    methods(Access = protected)
        ...
        function setupImpl(obj)
            ...
            % Load the occupancy grid into the system object.
            if ~isempty(obj.mapName)
                rawMap = evalin('base',obj.mapName);
                if isa(rawMap,'robotics.OccupancyGrid')
                    obj.map = rawMap;
                % If the map is a binary occupancy grid, convert it to a
                % probabilistic one. This provides access to the "raycast"
                % method, which is used to simulate lidar
                elseif isa(rawMap,'robotics.BinaryOccupancyGrid')
                    obj.map = robotics.OccupancyGrid(rawMap.occupancyMatrix, ...
                                                     rawMap.Resolution);
                    obj.map.GridLocationInWorld = rawMap.GridLocationInWorld;
                else
                    error('Map must be a robotics.OccupancyGrid or robotics.BinaryOccupancyGrid');
                end 
            end
